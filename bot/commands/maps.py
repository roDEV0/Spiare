...
class Maps(commands.Cog):
    ...
    async def player_heatmap(self, interaction: discord.Interaction, player: str):
        ...
        if not player_object:
            await interaction.followup.send(f"**{player}** is not registered")
            return
        ...
        grid = np.zeros((80, 160))

        for position in positions:
            translated_x = int((position[0] + 64512) / 129024 * 160)
            translated_z = int((position[2] + 32256) / 64512 * 80)
        ...
        black_mask = (r <= 10) & (g <= 10) & (b <= 10)
        data[:, :, 3] = np.where(black_mask, 0, a)

        heatmap_img = Image.fromarray(data, "RGBA")

        full_map = Image.open("shared/map.png")
        print(full_map.filename)
