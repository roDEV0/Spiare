...
class Settings(commands.Cog):
    ...
    async def set_citizen_role(self, interaction: discord.Interaction, role: discord.Role):
        ...
        await config.save()
        await interaction.response.send_message(f"Citizen successfully changed to {role.mention}")
    ...
    async def set_admin_role(self, interaction: discord.Interaction, role: discord.Role):
        ...
        await config.save()
        await interaction.response.send_message(f"Admin successfully changed to {role.mention}")

    @set_group.command(name="foreigner_role", description="Sets the foreigner role")
    async def set_foreigner_role(self, interaction: discord.Interaction, role: discord.Role):
        config, _ = await Config.get_or_create(server_id=role.guild.id)
        config.name = role.guild.name
        config.foreigner_role_id = role.id
        await config.save()
        await interaction.response.send_message(f"Foreigner successfully changed to {role.mention}")
    ...
    async def set_verify_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        await config.save()
        await interaction.response.send_message(f"Verify channel successfully changed to {channel.mention}")

    @set_group.command(name="notifications_channel", description="Sets the notifications channel")
    async def set_notif_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        await config.save()
        await interaction.response.send_message(f"Notifications channel successfully changed to {channel.mention}")
    ...
    async def set_nation(self, interaction: discord.Interaction, nation: str):
        ...
        await config.save()
        await interaction.response.send_message(f"Nation successfully changed to {nation}")