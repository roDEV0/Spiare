...
class Citizens(commands.Cog):
    ...
    async def check_citizens(self):
        nation_data = await self.bot.http_requester.post_request(
            "nations", self.bot.nation
        )
        ...
        new_citizens = list(
            set(updated_list) - set(active_citizens)
        )  # Players who have joined
        lost_citizens = list(
            set(active_citizens) - set(updated_list)
        )  # Players who have left via /t leave
        ...
        for citizen in active_citizens:
            ...
            if self.current_time - player_data["timestamps"]["lastOnline"] > (
                60 * 60 * 24 * 42
            ):  # 42 days
    ...
    async def update_inactive(self):
        ...
        for inactive in self.inactive_list:
            player_data = await self.bot.http_requester.post_request(
                "players", inactive
            )
            if self.current_time - player_data["timestamps"]["lastOnline"] < (
                60 * 60 * 24 * 42
        ...
        self.inactive_list = [
            c for c in self.inactive_list if c not in returning_citizens
        ]
